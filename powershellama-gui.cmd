<# : batch launcher
@echo off
powershell -ExecutionPolicy Bypass -WindowStyle Hidden -Command "& {iex (Get-Content '%~f0' -Raw)}"
exit /b
#>
    ...
    if ($line.StartsWith(",")) {
        ...
        try {
            ...
            elseif ($query.StartsWith('analyze ')) {
                ...
                Write-Terminal "Analyzing $($filesData.Count) files..." ([System.Drawing.Color]::Gray)
                $resp = Invoke-ShellamaAPI "/analyze" @{ files = $filesData; model = $model }
                if ($resp.analysis) { Write-Terminal $resp.analysis ([System.Drawing.Color]::Cyan) }
            }
